...
from school_info.models.model import newsBase, role_model_Base
...
async def create_role_model(
    ...
    #生成显示公告url
    download_url = f"http://{HOST}:{PORT}/school_info/show_role_model?file_path={file_path}"
    #公告信息存入数据库
    db_bulletin = role_model_Base(unique_filename=unique_filename, file_path=file_path,file_show_url=download_url)
    db.add(db_bulletin)
    db.commit()
...
# 删除公告（同时删除关联文件）
@router.delete("/delete_role_model/")
def delete_bulletin(unique_filename:str,db: Session = Depends(get_db)):
    db_role_model= db.query(role_model_Base).filter(role_model_Base.unique_filename == unique_filename).first()
    if db_role_model is None:
        raise HTTPException(status_code=404, detail="公告不存在")

    # 删除关联文件
    delete_associated_file(db_role_model.file_path)
    #删除公告
    db.delete(db_role_model)
    db.commit()
