...
from school_info.models.model import newsBase, role_model_Base, dynamicBase
...
# 上传公告（支持文件上传）
@router.post("/upload_school_dynamic/")
async def create_role_model(
    ...
    #生成显示公告url
    download_url = f"http://{HOST}:{PORT}/school_info/show_school_dynamic?file_path={file_path}"
    #公告信息存入数据库
    db_bulletin = dynamicBase(unique_filename=unique_filename, file_path=file_path,file_show_url=download_url)
    db.add(db_bulletin)
    db.commit()
...
# 公告传输
@router.get("/show_school_dynamic/")
...
# 删除公告（同时删除关联文件）
@router.delete("/delete_school_dynamic/")
def delete_bulletin(unique_filename:str,db: Session = Depends(get_db)):
    db_school_dynamic= db.query(dynamicBase).filter(dynamicBase.unique_filename == unique_filename).first()
    if db_school_dynamic is None:
        raise HTTPException(status_code=404, detail="公告不存在")

    # 删除关联文件
    delete_associated_file(db_school_dynamic.file_path)
    #删除公告
    db.delete(db_school_dynamic)
    db.commit()
